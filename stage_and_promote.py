...
#  Run the specified route, wait for it to complete, then promote it
...
def valid_route(s):
    ...
    if routeName in promotable_routes:
        return routeName
    else:
        msg = "Unknown route: " + s
        raise argparse.ArgumentTypeError(msg)
...
def parse_args():
    ...
    parser.add_argument("route", type=valid_route,
            help="The route to run synchronously. One of {}".format(promotable_routes))
